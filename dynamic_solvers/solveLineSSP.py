...
class LineSSPChain:
    ...
    def create_policy_maps(self, sensor_states: List[int]) -> List[List[z3.ArithRef]]:
        ...
        sensor_to_action.append([Real(f'x⊥{act}', self.ctx) for act in self.actions])
        print(sensor_to_action)
    ...
    def extend_fully_observable_pomdp_constraints(self, exp_rewards: List[z3.ArithRef]):
        ...
        """
        print('#A POMDP cannot perform better than the fully observable case\n')
    ...
    def build_cost_reward_equations(self, ExpRew, X, Y):
        ...
        for s in range(self.size - 1):
            if s == self.goal:
                ...
                cost_equations.append(ExpRew[s] == equation) # Generate sum of pi variables for non-target states
    ...
    def build_budget_constraint(self, Y: List[z3.ArithRef], budget: int):
        ...
        budget_constraint = sum(Y) <= budget
